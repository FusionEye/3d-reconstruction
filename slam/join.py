...
def joinPointCloud(pointCloud, color, RGBFileName, DepthFileName, lastFrame, CameraIntrinsicData,
...
def visualOdometry():
    ...
    camera = slamBase.CameraIntrinsicParameters(CameraIntrinsicData[0][2], CameraIntrinsicData[1][2],
                                                CameraIntrinsicData[0][0], C[1][1], 1000.0)
    frame = slamBase.Frame(path.join(RGBFileNamePath, str(START_INDEX) + '.png'),
                           path.join(DepthFileNamePath, str(START_INDEX) + '.png'))
    p, colors = slamBase.imageToPointCloud(path.join(RGBFileNamePath, str(START_INDEX) + '.png'),
                                           path.join(DepthFileNamePath, str(START_INDEX) + '.png'), camera)
    pcd = pcl.PointCloud()
    pcd.from_array(array(p, dtype=float32))
    for i in range(START_INDEX, END_INDEX):
        try:
            pcd, frame, colors = joinPointCloud(pcd, colors, path.join(RGBFileNamePath, str(i + 1) + '.png'),
                                                path.join(DepthFileNamePath, str(i + 1) + '.png'), frame, C,
                                                DistortionCoefficients, camera)
        except:
            pass

    pcl.save(pcd, OutputCloudFilename, format='pcd')
